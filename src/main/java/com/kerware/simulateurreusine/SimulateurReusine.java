...
public class SimulateurReusine {
    ...
    public int calculImpot(int revNetDecl1,
                           int revNetDecl2,
                           SituationFamiliale sitFam,
                           int nbEnfants, int nbEnfantsHandicapes,
                           boolean parentIsol) {

        verifierPreconditions(revNetDecl1, revNetDecl2,
                sitFam, nbEnfants, nbEnfantsHandicapes, parentIsol);
     ...
     */
    private void verifierPreconditions(int revNetDecl1, int revNetDecl2,
                                       SituationFamiliale sitFam,
                                       int nbEnfants, int nbEnfantsHandicapes,
                                       boolean parentIsol) {
        ...
        if (parentIsol && estCouple(sitFam)) {
            throw new IllegalArgumentException("Un parent isolé ne peut pas être marié ou pacsé");
        }
        if (estSeul(sitFam) && revNetDecl2 > 0) {
            throw new IllegalArgumentException(
                    "Un célibataire, un divorcé ou un veuf ne peut pas avoir de revenu pour le déclarant 2");
        }
    }

    private boolean estCouple(SituationFamiliale sitFam) {
        return sitFam == SituationFamiliale.MARIE || sitFam == SituationFamiliale.PACSE;
    }

    private boolean estSeul(SituationFamiliale sitFam) {
        return sitFam == SituationFamiliale.CELIBATAIRE
